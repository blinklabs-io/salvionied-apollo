...
func TestMinLovelacePostAlonzo(t *testing.T) {
	...
	minLov, err := MinLovelacePostAlonzo(&output, 4310)
	if err != nil {
		t.Fatal(err)
	}
	if minLov < 1_000_000 {
		t.Errorf("expected at least 1000000, got %d", minLov)
	}
}
...
func TestNewNativeScriptAny(t *testing.T) {
	...
	any, err := NewNativeScriptAny([]common.NativeScript{ns1, ns2})
	if err != nil {
		t.Fatal(err)
	}
	cborBytes, err := cbor.Encode(&any)
	if err != nil {
		t.Fatal(err)
	}
	if len(cborBytes) == 0 {
