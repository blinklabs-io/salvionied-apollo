...
func NewBursaWalletWithPassphrase(mnemonic string, passphrase string, opts ...bursa.WalletOption) (*BursaWallet, error) {
	...
	// Prepend WithPassword so the address derivation uses the same passphrase as key derivation.
	allOpts := append([]bursa.WalletOption{bursa.WithPassword(passphrase)}, opts...)
