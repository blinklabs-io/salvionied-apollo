...
func (pd *PlutusData) UnmarshalJSON(value []byte) error {
	...
	default:
		// Unhandled JSON type - ignore to maintain backwards compatibility
	}
	return nil
}
...
func (pd *PlutusData) handleGenericValue(x any, value []uint8) error {
	...
	if valid {
		...
		case map[any]any:
			...
			pd.TagNr = 0
		default:
			// nil and other unrecognized types represent empty PlutusData.
			// Initialize to an empty bytes value to avoid leaving pd in an undefined state.
			pd.PlutusDataType = PlutusBytes
			pd.Value = []byte{}
			pd.TagNr = 0
		}
