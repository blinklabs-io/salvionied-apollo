...
var validTestAddrBech32 string
...
func init() {
	...
	if err != nil {
		panic("failed to construct test address: " + err.Error())
	}
	validTestAddrBech32 = addr.String()
}
...
func TestPayToAddressWithReferenceScript(t *testing.T) {
	...
	a.PayToAddressWithReferenceScript(addr, 2_000_000, script)
	if len(a.payments) != 1 {
		t.Errorf("expected 1 payment, got %d", len(a.payments))
	}
}
...
func TestGetBurns(t *testing.T) {
	...
	if !burns.HasAssets() {
		t.Error("expected burns to have assets")
	}
}
...
func TestCompleteWithSetFee(t *testing.T) {
	cc := setupFixedContext()
	addr := testAddress(t)
	addTestUtxo(cc, addr, 10_000_000, 0x01, 0)
