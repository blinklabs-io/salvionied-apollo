...
// GreaterOrEqual returns true if v has at least as much coin and assets as other.
func (v Value) GreaterOrEqual(other Value) bool {
	...
	return v.Assets.Compare(other.Assets)
}
...
// The key must be 32 bytes (seed) or 64 bytes (extended key, first 32 bytes used as seed).
...
func ComputeScriptDataHash(
	...
	// Encode cost models as language views
	var costModelBytes []byte
	if len(costModels) > 0 {
		langViews := make(map[uint][]int64)
		...
		costModelBytes, err = cbor.Encode(langViews)
	} else {
...
func NewScriptRef(script common.Script) *common.ScriptRef {
	var scriptType uint
	switch script.(type) {
